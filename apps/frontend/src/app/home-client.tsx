...
function resolveStateBadgeVariant(
...
export function HomeClient() {
	...
	const handleConversationExpired = useCallback(
		(conversationId: string) => {
			router.replace(
				`/conversation-ended/${encodeURIComponent(conversationId)}`,
			);
		},
		[router],
	);
					...
					<Card>
						<CardHeader className="space-y-1 pb-2">
							...
							<CardDescription className="sm:block hidden">
								Нажмите на визуализатор, чтобы быстро выключить или включить
								микрофон.
							</CardDescription>
						</CardHeader>
						<CardContent className="flex-wrap flex gap-2 items-center">
							<Badge variant={resolveStateBadgeVariant(state)}>
								State: {resolveConversationStateLabel(state)}
							</Badge>
							...
							<Badge variant="outline">
								Session: {formatDuration(elapsedSec)}
							</Badge>
						</CardContent>
					</Card>
									...
									<div className="absolute inset-0 flex items-center justify-center pointer-events-none">
										<div className="backdrop-blur bg-card/80 shadow-xl px-4 py-2 border rounded-full">
											<p className="font-medium text-sm">
												{isMuted ? "Mic paused" : "Mic active"}
											</p>
											<p className="text-muted-foreground text-xs">
												{Math.round(visibleLevel * 100)}% input level
											</p>
										</div>
									</div>
								</div>
					...
					<Card className="h-[clamp(7.5rem,24vh,12rem)] shrink-0">
						<CardContent className="px-3 py-1.5 h-full">
							{feedMessages.length > 0 ? (
								<div
									className="pr-1 h-full overflow-y-auto"
									ref={conversationFeedScrollRef}
								>
										...
										{feedMessages.map((message) => (
											...
											>
												<p className="mb-0.5 text-[11px] text-muted-foreground">
													{resolveFeedRoleLabel(message.role)} •{" "}
								...
								<p className="bg-card/70 px-3 py-2 border rounded-xl text-muted-foreground text-sm">
									Сообщений пока нет. Начните говорить, чтобы увидеть ход
									диалога.
								</p>
							)}
						</CardContent>
				...
				<section className="space-y-4">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								История текущей сессии с авторами и временными метками.
							</CardDescription>
						</CardHeader>
						<CardContent>
							{feedMessages.length > 0 ? (
								...
								<p className="bg-card/70 px-3 py-3 border rounded-xl text-muted-foreground text-sm">
									История пуста. Сообщения начнут отображаться после первого
									успешного turn.
								</p>
							)}
						</CardContent>
				...
				<section className="gap-4 md:grid-cols-2 grid">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Базовые действия и текущее состояние аудио.
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-2">
							<div className="gap-2 grid-cols-2 grid">
								...
								<Button
									disabled={isMuted || micStatus !== "ready"}
									onClick={releaseUtteranceUrl}
								>
									Clear Recording
								</Button>
							</div>
							<div className="bg-muted/30 p-3 border rounded text-muted-foreground text-xs">
								capture: {captureStage} | mic:{" "}
								{resolveMicStatusLabel(micStatus)}
							</div>
							{lastUtterance ? (
								...
								<div className="gap-2 grid-cols-3 grid">
									{PERSONA_ORDER.map((persona) => (
										<Button
											...
											size="sm"
											variant={
												activePersona === persona ? "default" : "outline"
											}
										>
											{persona}
										</Button>
					...
					<Card>
						...
						<CardContent className="space-y-3">
							...
							>
								Language filter:{" "}
								{sttLanguageMode === "strict" ? "strict" : "off (faster)"}
							</Button>
							<div className="gap-2 grid-cols-2 grid">
								{STT_LANGUAGE_ORDER.map((language) => (
									<Button
										...
										size="sm"
										variant={
											selectedSttLanguages.includes(language)
												? "secondary"
												: "outline"
										}
									>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								VAD пресет и технические параметры.
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-3">
							<div className="gap-2 grid-cols-3 grid">
							...
							<div className="bg-muted/30 p-3 border rounded text-muted-foreground text-xs">
								threshold: {activeConfig.thresholdDb} dB | startHold:{" "}
								{activeConfig.startHoldMs} ms | endHold:{" "}
								{activeConfig.endHoldMs} ms
							</div>
							<div className="bg-muted/30 p-3 border rounded text-muted-foreground text-xs">
								STT: {sttLatencyMs ?? "--"} ms | LLM: {llmLatencyMs ?? "--"} ms
								| TTS: {ttsLatencyMs ?? "--"} ms
							</div>
						</CardContent>
					</Card>
			...
			{(lastNotice || lastError) && activeSection !== "settings" ? (
				<Card>
					<CardContent className="space-y-2 py-2">
						{lastNotice ? (
							<p className="bg-muted/40 px-3 py-2 border rounded text-muted-foreground text-sm">
								{lastNotice}
							</p>
						) : null}
						{lastError ? (
							<p className="px-3 py-2 border rounded text-destructive text-sm">
								{lastError}
							</p>
						) : null}
					</CardContent>
