...
function parseBody(rawBody: unknown): ChatRequestBody | null {
	...
	const personaId =
		personaRaw && isPersonaId(personaRaw) ? personaRaw : undefined;
...
function mapProviderError(status: number | null, message: string) {
	if (isLikelyProviderNetworkError(status, message)) {
		return {
			...
			message:
				"Cannot reach LLM provider. Check OPENAI_BASE_URL and model availability.",
		};
	}
...
export async function handleChatPost(request: Request) {
	...
	const history = (
		serverHistory.length > 0 ? serverHistory : (body.history ?? [])
